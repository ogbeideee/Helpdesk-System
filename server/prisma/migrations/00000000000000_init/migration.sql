...
-- One lead per assignment group: at most one TeamMembership row with
-- isLead = true per teamId. PostgreSQL boolean literal is required here
-- (SQLite's "WHERE isLead = 1" integer comparison is invalid on PostgreSQL).
